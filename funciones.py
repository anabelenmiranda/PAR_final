...
def viaticos_archivo():
    nombre = input("Ingrese nombre del archivo de viáticos (sin la extensión): ")
    ...
    with open("Clientes.csv", 'r', encoding="utf-8") as f_clientes:
        ...
        if opcion == 1:
            ...
            with open("viajes.csv", 'r', encoding="utf-8") as f_viaticos:
                ...
                lista_cliente = [', '.join(cliente) for cliente in lista_empresas if cliente[2] == in_dni]
...
def comprobacion_clientes(nombre_archivo):
    ...
    with open(nombre_archivo, 'r', encoding="utf-8") as csvfile:
        ...
        lista_clientes_actual = [[Nombre, Direccion, Documento, Fecha, Correo, Empresa] for Nombre, Direccion, Documento, Fecha, Correo, Empresa in lista_c_copia if Correo.count("@") == 1 and Correo.count(".") == 1]
        
        flag = True

        clientes_mail = [[Nombre, Direccion, Documento, Fecha, Correo, Empresa] for Nombre, Direccion, Documento, Fecha, Correo, Empresa in lista_c_copia if Correo.count("@") != 1 and Correo.count(".") != 1 and Correo != "Correo Electrónico"]
